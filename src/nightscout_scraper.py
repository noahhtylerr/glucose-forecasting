...
def _headers():
    # Nightscout expects the SHA-1 hex digest of the secret, never the secret itself
...
# Nightscout returns the most recent 'count' records, so a wide window silently
# drops the oldest data -- pull one day at a time to stay well under the cap
